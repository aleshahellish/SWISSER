...
export function createSwisserMcpServer() {
  const server = new McpServer({
    ...
    instructions:
      ...
      "для каждого обнови scanner и snapshot. Для готового входа потребуй свежий значимый LuxAlgo internal 1m CHoCH; " +
      "последующий BOS, displacement и завершённый откат/ретест усиливают подтверждение. Если последним событием уже " +
      "стал BOS, проверь, что он продолжает ту же CHoCH-цепочку. C2/C3 — только дополнительный контекст, не обязательный " +
      "триггер и не veto. Проверь также опоздание и инвалидацию, затем дай статус ВХОД ПОДТВЕРЖДЁН, ЖДАТЬ или ОТМЕНА. Если предыдущего списка нет, " +
