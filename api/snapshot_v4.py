...
def request_json(path: str, params: dict | None = None):
    ...
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 MEXC-Live-Analyst-V4/1.0",
...
def swing_structure_summary(
    ...
    active_highs.sort(key=lambda point: point["level"])
    active_lows.sort(key=lambda point: point["level"], reverse=True)
    ...
    return {
        ...
        "method": "confirmed_swings_with_level_state",
        ...
        "nearest_active_swing_high_above_price": (
            active_highs[0] if active_highs else None
        ),
        "nearest_active_swing_low_below_price": (
            active_lows[0] if active_lows else None
        ),
        "active_swing_highs_above_price": active_highs[:recent_limit],
        "active_swing_lows_below_price": active_lows[:recent_limit],
...
def build(symbol: str) -> dict:
    ...
    return {
        ...
        "mode": "enhanced_snapshot_v4",
        "version": "4.0-swing-level-states",
        "symbol": symbol,
