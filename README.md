...
Ручная кнопка закрепления сохранена как резервный вариант. Интерфейс панели монохромный и компактный. Три кнопки теперь
обозначают разные по глубине режимы и отправляют самодостаточные команды:

1. `Быстрый обзор` — только общий scanner и одна краткая таблица.
2. `Сделки сейчас` — scanner и snapshot всех реально подходящих кандидатов без
   жёсткого лимита.
3. `Шансы на вход` — scanner и snapshot всех кандидатов, близких к формированию
   в ближайшие часы; календарь и новости только при действительно значимом
   событии.

Лёгкий режим нельзя автоматически расширять до более тяжёлого.
...
готовые данные в фиксированную HTML-карточку. Верхняя таблица всегда показывает
все шесть монет, а нижняя появляется только для реальных кандидатов в режимах
2–3. Колонки, компактность, Italianno, приглушённые Bull/Bear и ранги
шампань/серебро/бронза зафиксированы; RR не выводится. Под итогом карточки
