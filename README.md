...
Дополнительно `/mcp` предоставляет ChatGPT scanner, подробный snapshot и панель
SWISSER с тремя быстрыми рыночными командами. Кнопки отправляют полные запросы
в текущий чат; панель автоматически запрашивает компактный PiP-режим и остаётся
поверх переписки. Ручная кнопка закрепления сохранена как резервный вариант.
Интерфейс панели монохромный и компактный. Три кнопки теперь обозначают разные
по глубине режимы и отправляют самодостаточные команды:
...
Модули `candle_closure.py`, `luxalgo_structure.py` и `mtf_decision.py` лежат в
корне и не создают отдельных функций. Откат выполняется через историю Git; дополнительная
