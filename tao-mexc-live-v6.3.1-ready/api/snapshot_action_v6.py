...
def _action_point(point):
    ...
    return {
        ...
        "confirmed_at_utc": point.get("confirmed_at_utc"),
        "current_state": point.get("current_state"),
    }


def _action_break(event):
    if not isinstance(event, dict):
        return None

    broken = event.get("broken_swing") or {}
    break_candle = event.get("break_candle") or {}
    protected = event.get("protected_level_created") or {}
    ...
    return {
        ...
        "time_utc": event.get("time_utc"),
        "broken_swing": {
            "type": broken.get("type"),
            "level": broken.get("level"),
            "confirmed_at": broken.get("confirmed_at"),
        },
        "break_candle": {
            "time": break_candle.get("time"),
            "open": break_candle.get("open"),
            "high": break_candle.get("high"),
            "low": break_candle.get("low"),
            "close": break_candle.get("close"),
        },
        "protected_level_created": {
            "protected_type": protected.get("protected_type"),
...
def _action_protection(level):
    if not isinstance(level, dict):
        return None

    return {
        "protected_type": level.get("protected_type"),
        "level": level.get("level"),
        "time": level.get("time"),
        "time_utc": level.get("time_utc"),
        "confirmed_at": level.get("confirmed_at"),
        "confirmed_at_utc": level.get("confirmed_at_utc"),
        "created_at": level.get("created_at"),
        "created_at_utc": level.get("created_at_utc"),
        "current_state": level.get("current_state"),
        "invalidation_rule": level.get("invalidation_rule"),
        "first_close_invalidation": level.get(
            "first_close_invalidation"
        ),
    }


def _action_cisd(event):
    if not isinstance(event, dict):
        return None

    anchor = event.get("anchor_swing") or {}
    confirmation = event.get("confirmation_candle") or {}

    return {
        "direction": event.get("direction"),
        "status": event.get("status"),
        "cisd_level": event.get("cisd_level"),
        "current_state": event.get("current_state"),
        "confirmed_at": event.get("confirmed_at"),
        "confirmed_at_utc": event.get("confirmed_at_utc"),
        "anchor_swing": {
            "type": anchor.get("type"),
            "level": anchor.get("level"),
            "confirmed_at": anchor.get("confirmed_at"),
        },
        "confirmation_close": confirmation.get("close"),
    }
...
def compact_snapshot_for_gpt_action(full_result: dict) -> dict:
    ...
    for timeframe, block in (
        ...
        swing_points = block.get("swing_points") or {}
        swing_structure = block.get("swing_structure") or {}
        protected = block.get("protected_structure") or {}
        breaks = protected.get("structure_breaks") or {}
        cisd = protected.get("cisd") or {}

        high_sequence = swing_structure.get("high_sequence") or {}
        ...
        output["timeframes"][timeframe] = {
            ...
            "swing_points": {
                ...
                "latest_swing_low": _action_point(
                    swing_points.get("latest_swing_low")
                ),
                "nearest_swing_high_above_price": _action_point(
                    swing_points.get(
                        "nearest_swing_high_above_price"
                    )
                ),
                "nearest_swing_low_below_price": _action_point(
                    swing_points.get(
            ...
            "swing_structure": {
                "provisional_swing_bias": swing_structure.get(
                    "provisional_swing_bias"
                ),
                "high_sequence": {
                    "status": high_sequence.get("status"),
                    "label": high_sequence.get("label"),
                    "levels": high_sequence.get("levels"),
                },
                "low_sequence": {
                    "status": low_sequence.get("status"),
                    "label": low_sequence.get("label"),
                    "levels": low_sequence.get("levels"),
                },
                "nearest_primary_swing_high_above_price": (
                    _action_point(
                        swing_structure.get(
            ...
            "protected_structure": {
                "structure_breaks": {
                    "current_structure_direction": breaks.get(
                        "current_structure_direction"
                    ),
                    "latest_structure_break": _action_break(
                        breaks.get("latest_structure_break")
                    ),
                ...
                "current_protected_low": _action_protection(
                    protected.get("current_protected_low")
                ),
                "current_protected_high": _action_protection(
                    protected.get("current_protected_high")
                ),
                "active_directional_protection": (
                    _action_protection(
                        protected.get(
                            "active_directional_protection"
                        )
                ...
                "cisd": {
                    "current_cisd_direction": cisd.get(
                        "current_cisd_direction"
                    ),
                    "latest_confirmed_cisd": _action_cisd(
                        cisd.get("latest_confirmed_cisd")
                    ),
                    "latest_effective_cisd": _action_cisd(
                        cisd.get("latest_effective_cisd")
                    ),
                    "latest_failed_cisd": _action_cisd(
                        cisd.get("latest_failed_cisd")
                    ),
                    "latest_pending_cisd": _action_cisd(
                        cisd.get("latest_pending_cisd")
                    ),
                    "recent_confirmed_cisd": [
                        _action_cisd(event)
                        for event in (
                    ...
                    "holding_count": cisd.get("holding_count"),
                    "failed_count": cisd.get("failed_count"),
                    "pending_count": cisd.get("pending_count"),
                },
            },
        }
